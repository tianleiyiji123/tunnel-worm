...
- **🔗 分享链接** — 一键生成短链接 `/s/{code}`，接收方点击即可提取
- **📷 二维码分享** — 发送成功后展示二维码，扫码直达提取页
- **🔐 端到端加密** — 可选开启，AES-256-GCM 浏览器端加密，服务端无法解密
- **🔐 密码保护** — 大写字母+数字混合密码（排除易混淆字符），5 次错误锁定
- **⏰ 自动过期** — 资源 24 小时后自动删除，登录用户文本永久保存
- **👤 可选登录** — 不登录也能用；登录后可查看操作记录
...
3. （可选）开启 **端到端加密**
4. 点击「生成密码」，获得 4 位密码
5. 通过 **密码**、**分享链接** 或 **二维码** 将资源分享给对方

### 提取资源

| 方式 | 操作 |
| --- | --- |
| **手动输入** | 输入 4 位密码 → 点击「提取资源」 |
| **分享链接** | 点击 `/s/{code}` 链接 → 自动提取并展示结果 |
| **二维码** | 扫码 → 自动跳转提取页并展示结果 |
...
发送方在发送前可开启「端到端加密」开关。启用后：

- 内容在**浏览器端**使用 AES-256-GCM 加密后上传
- **服务端只存储密文**，完全无法解密
- 接收方提取时浏览器自动解密，无需额外操作
- 密码错误则 GCM 认证失败，提示解密失败

加密性能几乎无感，文本加密 < 100ms，50MB 文件约 200ms。
...
- **端到端加密**: 可选 AES-256-GCM + PBKDF2 浏览器端加密，服务端无法解密
